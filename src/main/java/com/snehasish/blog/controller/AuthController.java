...
import com.snehasish.blog.payload.HttpResponseCustom;
...
//		return new ResponseEntity<JWTAuthenticationResponse>(jwtAuthenticationResponse, HttpStatus.OK);
    ...
    public ResponseEntity<HttpResponseCustom> registerUser(@Valid @RequestBody UserDto userDto) {
        UserDto registeredUser = this.userService.registerNewUser(userDto);
//		return new ResponseEntity<UserDto>(registeredUser, HttpStatus.CREATED);
        return ResponseEntity.created(URI.create("")).body(
                HttpResponseCustom.builder()
                        .timeStamp(LocalDateTime.now().toString())
                        .payload(Map.of("user", registeredUser))
